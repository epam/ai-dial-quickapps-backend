...
def filter_out(headers):
    ...
    return {key: value for key, value in headers.items() if key not in filtered_headers}


class CacheResponse:

    @classmethod
    def from_response(cls, response: Response, request: CacheRequest):
        body_encoded = ''
        try:
            body_str = response.content.decode()
        except UnicodeDecodeError:
            body_encoded = base64.b64encode(response.content).decode('utf-8')
            body_str = ''
        return cls(
            ...
            for chunk in chunks:
                if chunk.startswith("data: "):
                    ...
                    try:
                        ...
                        content = (
                            parsed_json.get("choices", [{}])[0].get("delta", {}).get("content", "")
                        )
                        if content:
                            cc = content.replace('\\n', '\n')
