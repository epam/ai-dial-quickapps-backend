...
@e2e_test(
    config_file_set="integration",
    test_case=TstCase(
        "Text-to-image2", "Image generation test case", similarity_threshold=0.7
    ).add_user_message(
...
@e2e_test(
    config_file_set="integration",
    test_case=TstCase(
        "Web search2", "How many Tesla cars were sold in 2024", similarity_threshold=0.7
    )
    .add_mock_date(date(2024, 12, 31))
    .add_user_message(
...
@e2e_test(
    config_file_set="integration",
    test_case=TstCase(
        "RAG search", "Search information in document", similarity_threshold=0.8
    ).add_user_message(
...
@e2e_test(
    ...
    contexts=[Path(__file__).parent / "test_documents/integration_context_items.txt"],
    test_case=TstCase(
        ...
        tool_calls=[
            ...
            ToolCall(ToolNames.INTERNAL_FILE_READ_LINES.value, max_calls=5).add_soft_argument_check(
                "path",
                ["integration_context_items", "files/"],
            ),
        ],
        answer=[
            "1st - GammaMapper\n2nd - BetaIndexer\n3rd - AlphaReader",
...
@e2e_test(
    ...
    test_case=TstCase(
        ...
        answer=["Elephant", "African Elephant", "Elephant."],
    ),
)
...
@e2e_test(
    ...
    test_case=(
        TstCase(
            "PyInterpreter3",
            "Python code interpreter use same session test",
        ...
        .add_user_message(
            ...
            tool_calls=[
                ToolCall(
                    ToolNames.PYTHON_CODE_INTERPRETER.value, max_calls=5
...
@e2e_test(
    ...
    test_case=TstCase(
        ...
        tool_calls=[
            ToolCall(ToolNames.CREATE_SHAPE_BOX.value).add_soft_argument_check(
                "box_data",
                ["""{"shapes": ["square", "triangle"], "capacity": 3}"""],
            ),
            ToolCall(ToolNames.ADD_SHAPE_TO_BOX.value).add_soft_argument_check("shape", ["circle"]),
            ToolCall(ToolNames.REMOVE_SHAPES_FROM_BOX.value).add_soft_argument_check(
                "shapes", ["square", "triangle"]
            ),
            ToolCall(ToolNames.GET_SHAPES_FROM_BOX.value).add_soft_argument_check(
                name="shape", value=["circle"]
            ),
        ],
        answer=["Circle is in the box, the colour of the circle is blue."],
    ),
)
...
@e2e_test(
    ...
    test_case=TstCase(
        ...
        tool_calls=[
            ToolCall(ToolNames.INVERT_STRING.value).add_soft_argument_check(
                "incoming",
                ["abrakadabra"],
            ),
            ToolCall(ToolNames.LIST_FROM_WORD.value, max_calls=3).add_soft_argument_check(
                "incoming", ["arbadakarba"]
            ),
        ],
        answer=[
            ...
            "json_schema": {
                ...
                "schema": {
                    ...
                    "properties": {
                        ...
                        "weapon": {"type": "string", "description": "The character's weapon."},
                        "friend": {"type": "boolean", "description": "Is the character a friend?"},
                    },
