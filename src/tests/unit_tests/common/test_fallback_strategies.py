...
# -- stop (deprecated): instruction-based with error forwarded ---------------


def test_stop_strategy_logs_deprecation_warning(caplog):
    with caplog.at_level(logging.WARNING, logger="quickapp.config.tools.tool_fallback"):
        StopStrategyModel()
    assert any(
        "stop" in r.message.lower() and "deprecated" in r.message.lower() for r in caplog.records
    )


def test_stop_catchall_returns_stop_instruction_with_error(caplog):
    err = ValueError("tool failed badly")
    with caplog.at_level(logging.WARNING, logger="quickapp.config.tools.tool_fallback"):
        result = FallbackProcessor.process_fallback([StopStrategyModel()], "c", err)
    assert "tool failed badly" in result.content
    assert "STOP" in result.content


def test_stop_with_matching_trigger_returns_stop_instruction(caplog):
...
def test_stop_with_non_matching_trigger_falls_through_to_continue(caplog):
...
def test_stop_with_timeout_trigger_returns_stop_instruction(caplog):
...
# -- hard_stop raises FallbackAgentStopException -----------------------------


def test_hard_stop_catchall_raises_fallback_stop_exception():
    err = ValueError("tool failed badly")
    with pytest.raises(FallbackAgentStopException):
        FallbackProcessor.process_fallback([HardStopStrategyModel()], "c", err)


def test_hard_stop_with_matching_trigger_raises():
    err = ToolErrorException("my_tool", "quota exceeded for this billing period")
    strategies = [
        HardStopStrategyModel(trigger_on=TriggerOn(type=TriggerOnType.contains, value="quota")),
        ContinueStrategyModel(),
    ]
    with pytest.raises(FallbackAgentStopException):
...
def test_hard_stop_with_non_matching_trigger_falls_through_to_continue():
    err = ToolErrorException("my_tool", "transient network error")
    strategies = [
        HardStopStrategyModel(trigger_on=TriggerOn(type=TriggerOnType.contains, value="quota")),
        ContinueStrategyModel(),
...
def test_hard_stop_with_timeout_trigger_raises():
    err = ToolTimeoutError("rag_search", 300)
    strategies = [
        HardStopStrategyModel(trigger_on=TriggerOn(type=TriggerOnType.contains, value="timed out")),
    ]
    with pytest.raises(FallbackAgentStopException):
        FallbackProcessor.process_fallback(strategies, "c", err)
