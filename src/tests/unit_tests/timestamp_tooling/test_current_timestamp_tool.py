...
def mock_time_provider():
    ...
    provider.tz_name = "UTC"
    provider.source = TimestampSource.SERVER
    return provider
...
@pytest.mark.asyncio
async def test_run_in_stage_returns_labeled_content(tool, mock_time_provider):
    result = await tool._run_in_stage_async(None)
    assert result.content == "2026-01-15T12:30:00+00:00 (UTC, source=server)"
    assert result.content_type == "text/plain"
    mock_time_provider.now.assert_called_once()


@pytest.mark.asyncio
async def test_arun_sets_tool_call_id(tool):
    result = await tool.arun("test_call_id")
    assert result.tool_call_id == "test_call_id"


@pytest.mark.asyncio
async def test_run_presets_metadata_in_state(tool):
    result = await tool._run_in_stage_async(None)
