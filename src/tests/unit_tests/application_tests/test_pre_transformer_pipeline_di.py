...
class TestUserAttachmentsPipelineDI(unittest.TestCase):
    def setUp(self):
        def configure(binder: Binder):
            binder.bind(
                ApplicationConfig,
                to=create_app_configuration([])
            )
    ...
    def test_pdf_attachment_text_metadata_injected(self):
        ...
        async def get_method(messages_setup: _MessagesSetup = Injected(_MessagesSetup)):
            ...
            self.assertIn("Attachment report.pdf", content)
            self.assertIn("application/pdf", content)
            self.assertEqual(len(result[0].custom_content.attachments), 0)

            return {"message": "success"}

        response = self.client.get("/test_pdf")
    ...
    def test_image_attachment_kept_inline_with_text_metadata(self):
        ...
        async def get_method(messages_setup: _MessagesSetup = Injected(_MessagesSetup)):
            ...
            self.assertEqual(len(result), 1)
            self.assertEqual(len(result[0].custom_content.attachments), 1)
            content = str(result[0].content)
