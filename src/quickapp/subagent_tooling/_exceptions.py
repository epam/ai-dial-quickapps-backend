...
class SubagentToolErrorException(ToolErrorException):
    ...
    would then answer from nothing — the same confabulation failure
    ``SubagentToolSetResolutionError`` exists to prevent. Raising routes the spawn
    through the ordinary tool-error path instead, where the coordinator can retry with
    a narrower task.
    """
