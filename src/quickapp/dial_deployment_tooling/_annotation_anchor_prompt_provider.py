...
Some tool responses embed citation anchors written as `<cit data-id="..."></cit>` directly after
the claim they support. These are **not** the internal `[id]` citations you are told to strip —
they are part of the answer and must survive into your response.

1. Copy each `<cit data-id="..."></cit>` tag **verbatim**, keeping it directly after the claim it
   supports.
...
class _AnnotationAnchorPromptProvider(PromptPartProvider):
    """Tells the orchestrator to preserve `<cit data-id="..."></cit>` anchors from tool responses.
