...
7. **Prefix with `[Preview]`** for features gated behind `ENABLE_PREVIEW_FEATURES`, and confirm the gate from source rather than the PR title. A feature is preview-gated when its code carries one of: `@preview_module` (the whole DI module is dropped when preview is off), `PreviewField(...)` on a config field, or `@preview_model` on a config model (field/model stripped from the schema and nullified at runtime when preview is off). Grep the feature's module and config for these markers **at the target tag** before tagging — the tag is a claim about a code-level gate, and that gate can shift between releases (see the Merge-path M4 warning about GA graduations that lift it mid-cycle).
...
- **Re-verify every `[Preview]` tag against source at the stable tag — don't trust the copied wording.** M3 copies rc bullets verbatim, which also copies each `[Preview]` tag as it was true *when that rc was authored*. But the tag records a code-level gate (§5 rule 7: `@preview_module` / `PreviewField` / `@preview_model`), and a GA-graduation PR in a *later* rc of the same cycle can lift that gate — so a bullet correctly tagged `[Preview]` in rc.0 goes stale the moment a subsequent rc graduates it. Treat every "Graduate `<X>` to GA" bullet as a checklist: find each other bullet touching `<X>` (its tools, sub-features, config, error handling, stage rendering) and strip `[Preview]` from those that are now GA. Confirm each one individually against source, though — a sub-feature can stay gated after its parent graduates, so don't flip a whole group blind. *Example (0.9.0): #377 graduated the DIAL files toolkit to GA, so the search/find enhancement (#352), the file-tool error-handling fix (#354), and the stage-rendering change (#365) all had to shed the `[Preview]` tag they carried in rc.0 — while the tool-call-result-offload sub-feature (#234) kept it, since it stays behind `PreviewField` even with its parent GA.*
...
- **Verify `[Preview]` tags against source gating at the target tag**, not the copied wording. `[Preview]` claims a code-level gate (`@preview_module` / `PreviewField` / `@preview_model`); a GA graduation in a later rc can lift it, so a tag that was right in an earlier rc goes stale at the stable cut. Re-check each one (§5 rule 7, Merge-path M4).
