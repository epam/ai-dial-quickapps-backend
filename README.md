...
| **Logging**                                |                            |          |                                                                                                              |
| `LOG_FORMAT`                               | See below ¹                | No       | Custom logging format string. Supports the `%(otel*)s` fields (`otelTraceID`, `otelSpanID`, `otelServiceName`, `otelTraceSampled`) which are populated when OTEL log correlation is enabled (`OTEL_PYTHON_LOG_CORRELATION=true`) and fall back to neutral defaults otherwise. |
