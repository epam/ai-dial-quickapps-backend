...
| **Subagents**                              |                            |          |                                                                                                              |
| `SUBAGENT_TIMEOUT_SECONDS`                 | `600.0`                    | No       | Wall-clock ceiling on one subagent spawn. An app's `features.subagents.timeout_seconds` may shorten this but never extend it. On expiry the spawn fails as a tool error the agent can retry. |
| `SUBAGENT_MAX_CONCURRENT_SPAWNS`           | `4`                        | No       | Maximum subagent spawns running at once in this replica, across all requests. Excess spawns queue rather than fail. |
| **Feature Gating**                         |                            |          |                                                                                                              |
