...
| `TOOL_CALL_RESULT_OFFLOAD__ENABLED_BY_DEFAULT` | `true`                 | No       | Whether the per-app offload section (`features.dial_files.tool_call_result_offload`) is present by default. Apps override per-app: presence enables it, `null` disables it. |
...
| `TOOL_CALL_RESULT_OFFLOAD__EXCLUDED_TOOLS` | `["internal_file_read_lines","internal_file_search"]` | No | Default JSON list of tool names exempt from offloading (the read-back tools, so a large read-back slice is never re-offloaded). Apps override per-app via `features.dial_files.tool_call_result_offload.excluded_tools`. |
| **External URL Egress**                    |                            |          |                                                                                                              |
