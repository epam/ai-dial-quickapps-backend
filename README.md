...
| `DIAL_FILE_MAX_DOWNLOAD_BYTES`             | `10485760`                 | No       | Deployment-wide default maximum size (in bytes) for files downloaded from DIAL Core when resolving file arguments for tool calls. Apps can override per-app via `tool_defaults.max_file_download_bytes`. |
| **Feature Gating**                         |                            |          |                                                                                                              |
