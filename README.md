...
| **Agent Skills**                           |                            |          |                                                                                                              |
| `SKILLS_FILE_MAX_BYTES`                    | `40000`                    | No       | Maximum size of any skill file returned to the agent, `SKILL.md` included, and the ceiling for a manifest read plus its file inventory together. Over the limit the read is refused rather than truncated — a half-file is worse than a clear failure for text the agent is about to follow as instructions. |
| `SKILLS_INVENTORY_MAX_ENTRIES`             | `200`                      | No       | Bound on a skill's bundled-file inventory. For a `dial-skill` resource it bounds both what is fetched from DIAL Core and what is shown, and a truncated inventory is marked as such so the agent is not told the skill holds nothing else. |
| `DIAL_SKILLS_MAX_CONFIGURED_SKILLS`        | `20`                       | No       | Maximum unique `dial-skill` URLs resolved per request. Counted after deduplication, so repeating a URL costs one slot; each unique URL beyond the cap is skipped and reported in the *Initialization issues* stage. |
| **Feature Gating**                         |                            |          |                                                                                                              |
