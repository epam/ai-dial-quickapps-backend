...
| **Tool Timeouts**                          |                            |          |                                                                                                              |
| `DEFAULT_TOOL_TIMEOUT_SECONDS`             | `300.0`                    | No       | Deployment-wide default timeout (seconds, `0 < x ≤ 3600`) applied to every tool call (deployment, REST API, MCP, Python interpreter, `DialCoreClient`). Apps can override per-app via `tool_defaults.timeout_seconds`. |
| **Feature Gating**                         |                            |          |                                                                                                              |
