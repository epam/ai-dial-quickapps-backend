...
**Post-completion `usage` on a Response (`ResponseUsage`)** — the `usage` block inside a full `responses.create` response; reference for future usage consolidation, not the count endpoint:
...
**Full Responses API response (`POST /v1/responses`)** — canonical shape adapters align token fields to. Example from the [Responses API reference](https://developers.openai.com/api/reference/resources/responses/); `usage` is the normalization target for post-completion token reporting:
...
### Proposed full normalized response (future usage consolidation)

The v1 **count** endpoint returns only `response.input_tokens` (input count). For a future endpoint or stream usage normalization, adapters would map provider post-completion usage into the same `usage` object shape as a full OpenAI Response — plus `provider_details` for fields that have no OpenAI equivalent.

**Proposed DIAL normalized `usage` (Anthropic deployment example):**
...
Mapping applied: `input_tokens` = `2095 + 2051 + 2051` (Anthropic sum rule); `output_tokens_details.reasoning_tokens` ← `thinking_tokens`; excessive Anthropic fields preserved in `provider_details`.

**Proposed DIAL normalized `usage` (Gemini deployment example):**
...
```json
{
  ...
  "provider_details": {
    "candidates_token_count": 52,
    "tool_use_prompt_token_count": 48
  }
}
```

Mapping applied: `output_tokens` = `52 + 20` (`candidates_token_count` + `thoughts_token_count`); `input_tokens_details.cached_tokens` ← `cached_content_token_count`; Gemini-only splits preserved in `provider_details`.

**Proposed DIAL normalized `usage` (OpenAI deployment — pass-through):**
...
No `provider_details` needed when the provider response already matches OpenAI `ResponseUsage`.
