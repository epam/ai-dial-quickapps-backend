...
- **Unified initialization reporting**: Tool-initialization errors and skill-resolution warnings surface in
  a single stage with one section per feature, through the existing `_InitializationErrorHandler`
  generalized to cover multiple issue types.
...
the field is stripped from the published JSON schema and any user-supplied value is nullified by
`_gate_preview_fields`.
...
it returns warnings alongside resolved skills so the caller — the initializer (see §4) — can route them to the
...
duplicate names — produce a `SkillResolutionWarning` with the prompt URL and a human-readable reason.
...
`_DialPromptSkillInitializer`, and `_DialPromptSkillsContext` at `request_scope` (see §4), and exposes the
...
2. Add DIAL prompt skills from the context. If a DIAL prompt skill has the same `name` as a predefined
   ...
   `SkillResolutionWarning` that is pushed into the context (so the unified handler in §4.4 picks it up).
...
detects them during its merge and appends them to the same `_DialPromptSkillsContext`. Because the
unified `_InitializationErrorHandler` (§4.4) reads from the context, collision warnings reach the user in
the same `Skill loading` section as fetch/validation warnings and alongside any tool-initialization
errors — one consolidated UI surface. The pipeline reshuffle in §4.1 is what makes this safe: the
registry's merge runs during message finalization, and the handler runs *after* finalization.
...
### 4. Eager resolution — pipeline reshuffle, initializer, context, and unified error handler

**Owner:** `dial_prompt_skills/` package for the new components; `application/` package for the pipeline
reshuffle and the generalized `_InitializationErrorHandler`. The wiring mirrors MCP tooling.
...
finalization handler cover both tool initialization errors and skill resolution warnings (§4.4).
Everything else in this section depends on the reshuffle being in place.
...
| `invoke_initializers(InitializerType.completion)` | run initializers — including `_DialPromptSkillInitializer` and existing tool initializers; each populates its own feature context | — |
| `_RequestContextSetup.finalize_messages()` | internally calls `_MessagesSetup.setup()` and stores the transformed messages on `_RequestContext`; runs every `MessagesTransformer`, including `_AddSystemPromptTransformer` which produces the system prompt (and may push predefined-vs-external collision warnings into `_DialPromptSkillsContext`) | after initializers |
...
the initializer catches the exception and records a dedicated *catastrophic* entry — see §4.3's
`catastrophic_failure` field — then returns. The request proceeds with predefined skills only. This matches
the graceful-degradation semantics in §2.
...
loaded) is captured in the `_catastrophic_failure` field. The unified handler (§4.4) renders them under
different headers within its `Skill loading` section, and treats `catastrophic_failure` as a **hard**
issue that flips the stage's close status to `FAILED`. This removes the `url=""` sentinel from the
previous revision — no magic values, no special-casing beyond "does this field exist?".

`DialPromptSkillsModule` exposes the warnings list via `@multiprovider -> list[SkillResolutionWarning]`
for future consumers (metrics, logs, tests). The unified `_InitializationErrorHandler` does not use that
binding — it injects `_DialPromptSkillsContext | None` directly (same optional-injection style as
`SkillsRegistry`) and reads `context.warnings` / `context.catastrophic_failure`. This mirrors how
`_MCPToolingContext.exceptions` is exposed through the context object itself.

**Collision warnings from `SkillsRegistry`.** The registry appends predefined-vs-external name collisions
to the context's warnings list during its merge (see §3). Because the merge runs in message finalization
(§4.1) and the handler runs *after* finalization (§4.1 table), these collisions surface alongside
fetch/validation warnings in the same `Skill loading` section — one consolidated UI surface.
...
appends to the same context and is rendered in the same `Skill loading` section at no extra wiring cost —
identical to how multiple tool initializers feed the same handler today.

#### 4.4 `_InitializationErrorHandler` — generalized for multiple feature sources
...
tool-init stage. The feature extends it so that one handler renders **one stage with one section per
feature that reported an issue**. Adding a new feature means adding one optional constructor dependency
and one rendering branch — no new handler, no new stage.

**Constructor dependencies after generalization.** Each feature contributes its own typed data source;
the handler is aware of them all.

| Dependency | Source | Used for |
|---|---|---|
| `ProviderOf[Stage]` | `Choice.create_stage()` | opens the unified stage |
| `ProviderOf[list[ToolInitializationException]]` | populated by `_MCPToolingContext.append_exception` | Tool initialization section |
| `_DialPromptSkillsContext \| None` | populated by `_DialPromptSkillInitializer` and `SkillsRegistry` merge; `None` when preview is disabled | Skill loading section |

Future features follow the same pattern — add an optional dependency, add a rendering branch. The
alternative of a shared cross-feature "issue bag" was considered and rejected: it would require a common
`InitializationIssue` supertype and a breaking change to every existing initializer, while the per-
feature-dependency approach matches the MCP context pattern already in the codebase.

**Single method, `handle_initialization_issues()`**:

1. Collect each feature's data (tolerating absent bindings for optional sources).
2. Short-circuit to a no-op if every source is empty/`None` — no stage is opened.
3. Otherwise open one `Stage` named **`"Initialization issues"`** and write markdown containing a
   `####`-level section per feature that has content:
   - **Tool initialization** — bullet per `ToolInitializationException` (`**{tool_name}{toolset_name}**:
     {message}`), followed by a fenced `details` block when present. Same format as today.
   - **Skill loading** — if `catastrophic_failure` is set, render it first under a distinct sub-header
     indicating DIAL prompts as a whole could not be loaded (fallback to predefined-only is in effect);
     then bullets for per-URL warnings (`**{url}**: {reason}`).

   Sections for features that reported nothing are omitted entirely.
4. Close the stage with a status derived from the content:
   - **`FAILED`** iff any *hard* issue is present — non-empty
     `list[ToolInitializationException]` **or** `catastrophic_failure is not None`.
   - **`COMPLETED`** otherwise (for example, per-URL skill warnings without any tool or catastrophic
     failure). The request always proceeds in both cases; the status flag is purely a UI cue.

Pipeline slot: after `finalize_messages()`, before `Orchestrator.invoke()` — see §4.1. Placing the
handler after finalization is required so that predefined-vs-external collision warnings pushed into the
skill context by `SkillsRegistry` during the message-setup merge are visible to the handler. The handler
itself does not trigger resolution, merging, or any I/O.

**DI wiring when preview is disabled.** `DialPromptSkillsModule` is filtered out as before. With preview
off `_DialPromptSkillsContext` is not bound; the handler's optional dependency resolves to `None` and the
Skill loading section is silently omitted. Behavior with respect to tool initialization errors is
unchanged.
...
### Shared skill helpers (shipped in PR #247)

`parse_frontmatter()` (raising `SkillValidationError`), `generate_skills_xml()` / `escape_xml()`, and the
`SkillMetadata` model now live as standalone helpers in `skills/_frontmatter.py`, `skills/_xml.py`,
`skills/_skill_metadata.py`, and `skills/_exceptions.py`. `DialPromptSkillResolver` reuses
`parse_frontmatter` for DIAL prompt validation; `SkillsRegistry` owns XML generation.
...
> Prerequisites already on `development`: async `PromptPartProvider` / `MessagesTransformer` contracts
> (PR #246) and the extracted `_frontmatter.py` / `_xml.py` / `_skill_metadata.py` / `_exceptions.py`
> helpers (PR #247). The `Prompts` / `AsyncPrompts` resource in `ai-dial-client-python` ships as a
> separate release of that library.
...
| **`skills/_skills_registry.py`** (new) | Request-scoped merger of predefined + resolved DIAL prompt skills; generates the combined `<available_skills>` XML. Pushes predefined-vs-external collisions into the context as warnings. |
...
| **`dial_prompt_skills/`** (new package) | Resolver, initializer, request-scoped context, and `@preview_module` DI wiring (mirror of `MCPToolingModule`). |
| **`application/_initialization_error_handler.py`** | Generalize to one handler with per-feature sections: inject `_DialPromptSkillsContext \| None` alongside the existing tool-exceptions provider; render one `"Initialization issues"` stage with `#### Tool initialization` and/or `#### Skill loading` sections; close `FAILED` on any hard issue (tool exception or skill `catastrophic_failure`), otherwise `COMPLETED`. |
| **`application/_request_context_setup.py`** | Split into `setup_pre_init()` and `finalize_messages()` — see §4.1. |
| **`application/_quick_app_completion.py`** | New call order: `setup_pre_init` → initializers → `finalize_messages` → `_InitializationErrorHandler` → `Orchestrator.invoke` (§4.1). |
...
| **Tests** | `test_skills_registry.py` rewritten around `_DialPromptSkillsContext`; new tests for `_DialPromptSkillInitializer` and for the generalized `_InitializationErrorHandler` (no-op on empty report, COMPLETED on skill-only warnings, FAILED on tool-only errors, FAILED with both sections on mixed input). |
