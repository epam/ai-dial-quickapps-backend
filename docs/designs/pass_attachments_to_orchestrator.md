...
- **Allowlist surfaced to the model:** The **advertised** allowlist — `OrchestratorCapabilities.advertised_input_attachment_types` — is **published to the agent in two places** so it can plan without guessing. It defaults to the deployment's `input_attachment_types`; when the app sets `accepted_types`, that app list is advertised verbatim instead — it is the narrower intent by construction, and it is deliberately **not** filtered against the deployment's declared list first: correctly deciding whether two MIME *patterns* overlap (e.g. an app's `image/*` against a deployment's concrete `image/png`) is exactly the fiddly pattern-intersection problem the *enforcement* gate (`orchestrator_accepts_mime_type`) avoids by only ever comparing a concrete mime against each list.
  (1) The `internal_attachments_get_content` tool's function description and its `attachment_url` parameter description embed the advertised MIME patterns **verbatim** (wildcards like `image/*` preserved) as a **comma-joined human-readable string**. The static template description is appended with a sentence of the form: `Accepted MIME types: image/*, application/pdf, text/csv.` Rendering happens at registration time in `LazyOnDemandStrategyModule._provide_internal_tools` from the request-scoped `OrchestratorCapabilities.advertised_input_attachment_types`; the static `GET_CONTENT_TOOL_CONFIG` constant is treated as a template.
...
|------------------|---------------------|
| Internal tools | New **`internal_attachments_get_content`** when orchestrator accepts input attachments **and** either external URL fetching is policy-enabled or at least one request-visible file (admin context / expanded folder / user attachment) is accepted by `OrchestratorCapabilities.orchestrator_accepts_mime_type` (`matches_type` from `quickapp/common/utils.py`, against the deployment's `input_attachment_types`); list tool keeps existing admin-context gate. |
| App-level MIME narrowing | New optional `LazyOnDemandAttachmentStrategy.accepted_types` field; every MIME gate in the strategy requires a match against **both** the deployment's `input_attachment_types` and this app-level list (conjunction), enforced solely inside `OrchestratorCapabilities`; the advertised allowlist (tool description, fail payload) is narrowed to match. |
