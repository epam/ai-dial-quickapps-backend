...
- **URL normalization:** `normalize_attachment_url_argument` strips outer whitespace **and** a single leading `/`, so DIAL-emitted `/files/...` and `files/...` forms compare equal at the three sites that consume normalized URLs: `collect_get_content_allowed_urls` (when building the per-request allowlist), `_GetContentKeepPolicy.should_keep` (the `startswith("files/")` storage-path check), and `_GetContentTool._run_in_stage_async` (the allowlist membership check). A leading `//` is preserved as malformed (allowlist match by exact string is the safety net).
- **External URLs (promotion):** When external URL fetching is enabled, an allowed attachment whose url is an external `http(s)` reference (a user attachment or admin context, still validated against the same allow-set) is **downloaded and promoted to a durable DIAL file** before it reaches the orchestrator. The orchestrator attachment channel has no external-URL resolution of its own and only accepts DIAL `files/...` urls, so an unpromoted external url could not be fetched by the deployment. Promotion is centralised in `_AttachmentMaterializer` (wrapping `DialFilePromoter`, which enforces the two-tier external-fetch policy, the SSRF guard, the redirect cap, and the size limit) and is shared by **both** the synthetic injector and the explicit tool call. The model continues to see and pass the **original** url (in `<attachments>` metadata and the synthetic tool-call arguments); the promoted DIAL url rides only on the emitted attachment and is recorded in the request-scoped `PromotedAttachmentUrls` registry, which `_GetContentKeepPolicy` unions into its allow-set. The `startswith("files/")` storage-path check is therefore **preserved** as defense in depth — it now validates the promoted url. When promotion is blocked (policy / SSRF / size), the injector skips the attachment (so no misleading success pair is emitted) and the explicit tool call returns a retry error carrying `accepted_types`.
...
- **Allowlist surfaced to the model:** The orchestrator's `input_attachment_types` is **published to the agent in two places** so it can plan without guessing.
  ...
  (2) On rejection, `_GetContentTool._error_result` includes an `accepted_types` field as a **JSON array of raw patterns** (e.g. `"accepted_types": ["image/*", "application/pdf", "text/csv"]`) on **every** rejection path — not only the explicit "deployment does not accept this file type" branch. Rationale: `collect_get_content_allowed_urls` filters by MIME during allowlist construction, so MIME-incompatible files most often fail with the generic `"Unknown or disallowed attachment_url"` error rather than the explicit MIME-reject error; carrying `accepted_types` on every rejection gives the agent the same recovery signal regardless of which branch fires. When `input_attachment_types` is unset, the tool is never registered, so neither surface ever exposes an empty list to the model. Truncation: no cap is applied — `input_attachment_types` in practice contains a handful of MIME patterns; if it ever grows to dozens, the cap policy is a follow-up doc change rather than a guess made now.
