...
**Behavior:** Orchestrator calls the **list** tool, then **`internal_attachments_get_content`** with `attachment_url` set to the stable reference from the list (in **`file:url::...`** form). The product returns that single file as the tool result attachment plus a human-readable `content` summary and structured success fields (`status`, `attachment_url`, `title`, `type`) in `custom_content.state["_get_content_response"]`.
...
- **Semantics:** (1) User message may reference an admin file or a user-uploaded file. (2) Orchestrator calls list tool → JSON/text with titles, MIME types, stable paths/ids, descriptions; disclaimer remains admin-focused (user `<attachments>` are not listed there). (3) For **admin files**, orchestrator calls `internal_attachments_get_content` with `attachment_url` in **`file:url::...`** form (see **Tool message format** below). For **attachments on the last USER message**, the pipeline injects synthetic `internal_attachments_get_content` ASSISTANT/TOOL pairs (one per attachment URL) before the model call. (4) The product accepts any DIAL `files/` url (passthrough) or external `http(s)` url (promoted to a DIAL file) whose MIME the orchestrator accepts — there is no in-app url allow-set, since authorization is enforced upstream (DIAL Core for `files/`, the external-fetch policy for `http(s)`); success → tool result with one file attachment, a human-readable `content` summary, and structured fields in `custom_content.state`; failure → human-readable `content` summary plus structured fail fields in state, no attachment. (5) Later turns follow existing history semantics, subject to Concern 2 and Concern 5.
...
- **Owner:** Internal tool multiplexer (`AttachmentProcessingModule`) + orchestrator deployment prefetch (Concern 4); every MIME check funnels through the single choke point **`_AttachmentAcceptance.accepts_mime_type`** (itself backed by `quickapp.common.utils.matches_type`; no ad-hoc wildcard logic anywhere else). `OrchestratorCapabilities` stays a thin, Core-only wrapper exposing the deployment's raw `input_attachment_types`; `_AttachmentAcceptance` (in the `lazy_on_demand` strategy layer) wraps it and layers the app-level `accepted_types` narrowing on top.
- **Semantics:** The **available-context list** tool follows existing activation (`should_activate_context_tool` / admin file contexts) unless product narrows it. **`internal_attachments_get_content`** is registered (gating function: `should_enable_get_content_tool` in `_gating.py`, which takes both the request-scoped `OrchestratorCapabilities` and `_AttachmentAcceptance` directly rather than a raw MIME list) when the orchestrator deployment accepts input attachments (`input_attachment_types` non-empty) **and** either (a) **external URL fetching is policy-enabled** for the request — resolved via `ExternalUrlFetchPolicyResolver.resolve_reason() == "allowed"` — or (b) at least one request-visible file (admin context, expanded folder file, or user attachment) is accepted by `_AttachmentAcceptance.accepts_mime_type`. Rationale for (a): once external fetching is on, an attachment url can reach the agent loop through **any** channel — the system prompt, a skill, the user message, or a tool result — not just a request-visible file, so the tool cannot be gated on request-visible files alone; it is always offered (and the model passes whatever url it encounters). Otherwise the tool is **absent** from the tool list so the model cannot call it. The **synthetic injector** still applies the **same per-attachment MIME gate** before emitting a pair — attachments whose MIME isn't accepted by the deployment are skipped, so the message history never references a tool the model hasn't been given. Tool descriptions still instruct: for admin files, list then fetch.
- **App-level narrowing (`LazyOnDemandAttachmentStrategy.accepted_types`):** A QuickApp may declare an optional `accepted_types` MIME allowlist on its `attachment_strategy` config to narrow *below* the shared deployment's `input_attachment_types` — for example scoping a deployment that declares the `*/*` catch-all down to `image/*` for a visual-only app, or catching a deployment whose declared list is simply wrong. `_AttachmentAcceptance.accepts_mime_type` requires a MIME to match **both** the deployment's list and the app's list (a conjunction, not an intersected pattern set — deliberately, since intersecting `image/*` against `*/*` or another wildcard is fiddly and error-prone); the deployment's declared list remains a hard cap the app can only narrow, never widen. When unset (the default), behavior is unchanged from the deployment-only gate described above.
...
- **Allowlist surfaced to the model:** The **advertised** allowlist — `_AttachmentAcceptance.advertised_input_attachment_types` — is **published to the agent in two places** so it can plan without guessing. It defaults to the deployment's `input_attachment_types`; when the app sets `accepted_types`, that app list is filtered down to entries that overlap the deployment's declared list (a cheap top-level-type check, `_patterns_overlap` — not full pattern intersection) before being advertised, dropping any `accepted_types` entry the deployment could never accept at all (e.g. `application/*` when the deployment only declares `image/*`); an entry that does overlap (even loosely, like `image/*` against a deployment's concrete `image/png`) is kept verbatim, since the real *enforcement* gate (`accepts_mime_type`) still correctly resolves those concrete mimes — full pattern intersection is exactly the fiddly problem that gate avoids by only ever comparing a concrete mime against each list, and the advertise-time filter deliberately doesn't attempt it either.
  (1) The `internal_attachments_get_content` tool's function description and its `attachment_url` parameter description embed the advertised MIME patterns **verbatim** (wildcards like `image/*` preserved) as a **comma-joined human-readable string**. The static template description is appended with a sentence of the form: `Accepted MIME types: image/*, application/pdf, text/csv.` Rendering happens at registration time in `LazyOnDemandStrategyModule._provide_internal_tools` from the request-scoped `_AttachmentAcceptance.advertised_input_attachment_types`; the static `GET_CONTENT_TOOL_CONFIG` constant is treated as a template.
  (2) On rejection, `_GetContentTool._error_result` returns a `GetContentToolResponse.fail` payload: structured fields (`status: "Fail"`, `status_message`, `accepted_types` as an array of raw MIME patterns, sourced the same way from `advertised_input_attachment_types`) in `custom_content.state["_get_content_response"]`, plus a human-readable fail summary in `content` (via `content_summary()`), on **every** rejection path — not only the explicit "deployment does not accept this file type" branch (e.g. also the empty-argument and unsupported-scheme branches). Rationale: carrying `accepted_types` on every rejection gives the agent the same recovery signal regardless of which branch fires. When `input_attachment_types` is unset, the tool is never registered, so neither surface ever exposes an empty list to the model. Truncation: no cap is applied — `input_attachment_types` in practice contains a handful of MIME patterns; if it ever grows to dozens, the cap policy is a follow-up doc change rather than a guess made now.
- **Tool message format (`GetContentToolResponse`):** Every `internal_attachments_get_content` TOOL message carries structured data in **`custom_content.state["_get_content_response"]`** (Pydantic model `GetContentToolResponse` in `_get_content_tool_response.py`) and a **human-readable summary** in `content` (built by `GetContentToolResponse.content_summary()`). QuickApps reads structured fields via `GetContentToolResponse.from_state()`; the model reads `content` text plus live-turn attachment bytes on `custom_content.attachments`. Fields in state:
  - **`status`:** `"Success"` or `"Fail"`.
  - **Success (attachment present):** `attachment_url` (model-facing `file:url::files/...` reference), `title`, `type` (MIME). The structured attachment bytes ride separately on `custom_content.attachments`.
  - **Success (persisted history, attachment stripped — Concern 5):** same success fields plus **`status_message`** explaining that the file payload was removed from saved history and the model should call the tool again with the same `attachment_url` when page-level or verbatim content is needed. Both state and `content` summary are rewritten on persist.
  - **Fail:** `status_message` (human-readable reason) and `accepted_types` (orchestrator MIME allowlist). Success-only fields are omitted.
  - **URL convention:** Tool arguments and state `attachment_url` use the **`file:url::...`** reference form the model is taught to imitate. Internal code normalizes to a bare DIAL path (`files/...`, sometimes called *display url*) for comparison against attachment dicts and promotion; `_model_facing_attachment_url()` wraps on emit and `_display_url_from_attachment_url()` unwraps on read.
- **Change:** Extend internal tool multiplexer with conditional registration for `internal_attachments_get_content`; render the live allowlist into the tool/parameter descriptions in `_provide_internal_tools`; emit structured fail payloads via `_GetContentTool._error_result` (`GetContentToolResponse.fail` → state + `content_summary()`); sweep up the stale execution-order docstring in `test_pre_transformer_pipeline_di.py` when the rendering work lands; no change to external deployment tool contracts.
...
- **Semantics:** Recovery proceeds **only** when the error is an `openai.BadRequestError` **and** the error `message` or `body` contains an attachment-related signal (`attachment`, `unsupported file`, `invalid file`, `input_attachment_types`, `image_url`, `file type`). All other errors propagate unchanged — the orchestrator's retry budget (one) is preserved for cases the policy is authorized to handle. The policy additionally returns `False` when no USER message exists in the conversation, so rehydrated histories that lack a current-turn anchor are not retroactively rewritten. On recovery, get-content TOOL messages are rewritten to a structured **`GetContentToolResponse`** fail payload (`status: "Fail"`, fixed `status_message` from `_get_content_recovery_payload.py`) in `custom_content.state` plus a human-readable fail summary in `content`, and attachments are stripped so the orchestrator can retry without the offending file.
...
- **Stable URL parameter:** Prefer canonical storage path from configuration in list responses. Tool arguments and state `attachment_url` echo the reference in **`file:url::files/...`** form; the model passes that same string into `internal_attachments_get_content`. For user `<attachments>`, synthetic injection wraps the original url the user saw while the promoted DIAL url rides only on the attachment object.
...
| Deployment declares `input_attachment_types: ["*/*"]` (or another over-broad list); app sets `attachment_strategy.accepted_types: ["image/*"]` | `internal_attachments_get_content` is registered/allowed only for images; a user's `report.pdf` is not lazily loadable even though the deployment claims to accept anything |
...
4. Orchestrator: `internal_attachments_get_content` with `attachment_url` set to the stable reference from the list (e.g. `file:url::files/bucket/folder/sales.csv`) → receives TOOL message with human-readable success summary, structured state (`status`, `attachment_url`, `title`, `type`), and file attachment.
...
3. Attachment-processing transformer injects synthetic ASSISTANT/TOOL `internal_attachments_get_content` pair with `attachment_url=file:url::files/bucket/uploads/customer-report.pdf`.
4. Backend validates URL against user attachments in current messages + MIME gate; TOOL message carries the attachment, a human-readable `content` summary (e.g. `Loaded file "customer-report.pdf" (application/pdf) from file:url::files/bucket/uploads/customer-report.pdf.`), and matching structured fields in `custom_content.state["_get_content_response"]`.
5. Orchestrator continues with normal completion loop using consistent tool history.

Step-by-step (follow-up turn after history persistence):

1. Prior turn successfully loaded `handbook.pdf` via `internal_attachments_get_content`; history was persisted after a terminal assistant message.
2. Rehydrated TOOL row for that call has **no** `custom_content.attachments`, but its human-readable `content` and `custom_content.state["_get_content_response"]` still carry `attachment_url`, `title`, `type`, and a **`status_message`** instructing the model to call the tool again when verbatim content is needed.
3. User asks a new question that requires the document again → orchestrator re-calls `internal_attachments_get_content` with the same `attachment_url` rather than assuming the file bytes are still in-thread.
...
Adding a **new** internal tool next to the list tool is **additive** if default prompt injection is unchanged. Tool results now store structured **`GetContentToolResponse`** data in `custom_content.state["_get_content_response"]` with a human-readable `content` summary instead of JSON in `content`. Further token savings from changing how often synthetic context notifications are injected are **out of scope for this document** (see attachment-notification path and product flags if revisited).
...
|------------------|---------------------|
| Internal tools | New **`internal_attachments_get_content`** when orchestrator accepts input attachments **and** either external URL fetching is policy-enabled or at least one request-visible file (admin context / expanded folder / user attachment) is accepted by `_AttachmentAcceptance.accepts_mime_type` (`matches_type` from `quickapp/common/utils.py`, against the deployment's `input_attachment_types`); list tool keeps existing admin-context gate. |
| App-level MIME narrowing | New optional `LazyOnDemandAttachmentStrategy.accepted_types` field; every MIME gate in the strategy requires a match against **both** the deployment's `input_attachment_types` and this app-level list (conjunction), enforced in `_AttachmentAcceptance` (the strategy layer wrapping the Core-only `OrchestratorCapabilities`); the advertised allowlist (tool description, fail payload) is filtered — not verbatim — down to entries that overlap the deployment's declared list. |
...
| Tool semantics | List remains admin-only; `internal_attachments_get_content` accepts any DIAL `files/` url (passthrough) or external `http(s)` url (promoted) whose MIME the orchestrator accepts — authorization is enforced upstream (DIAL Core / external-fetch policy), not by an in-app url allow-set; returns one attachment plus human-readable `content` and structured state (`GetContentToolResponse`: `status`, `attachment_url`, `title`, `type` on success; `status_message` + `accepted_types` on failure). |
| History persistence | `_GetContentHistoryPolicy` strips get-content attachments from persisted `tool_execution_history` and rewrites TOOL `content` and state with `status_message` so later turns know to re-call the tool. |
