...
**Behavior:** The file is not returned by `internal_attachments_available_context` (admin-only by contract), but `internal_attachments_get_content` accepts the exact user attachment URL (for supported MIME) and returns it as a tool-result attachment.

**Outcome:** User-uploaded files can be materialized lazily on the same path as admin files without broadening list-tool scope.
...
| User attaches their own file (e.g. their own PDF or photo) | Not listed by admin context tool; model can call `internal_attachments_get_content` directly with exact URL from `<attachments>` when MIME is supported |
...
2. Message `<attachments>` contains that exact URL and MIME.
3. Orchestrator calls `internal_attachments_get_content` with `attachment_url=files/bucket/uploads/customer-report.pdf`.
4. Backend validates URL against user attachments in current messages + MIME gate.
5. Tool returns one attachment; orchestrator continues with normal completion loop.
...
| Orchestrator input path | Narrow exception so **`internal_attachments_get_content`** results can retain allowed file types only when URL/MIME pass request-level admin/user checks and DialCore `input_attachment_types`; user images and other tools unchanged by policy. |
