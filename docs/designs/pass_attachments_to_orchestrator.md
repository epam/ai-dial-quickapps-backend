...
- **Transformer ordering:** The contract the model sees is the **message-list order**: all synthetic ASSISTANT/TOOL pairs land immediately before the last USER message. Later transformers insert closer to that USER, so with the current DI registration order (`AttachmentProcessingModule` before `LazyOnDemandStrategyModule`) the final list is context-notification → get-content → last USER. Two message transformers run during request setup: `_AttachmentGetContentInjector` (synthetic user-attachment get-content pairs) and `_AttachmentNotificationInjector` (context-list synthetic notification). Both use the same insertion site (immediately before the last USER); `test_pre_transformer_pipeline_di.py` pins the resulting message-list order.
